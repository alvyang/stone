/* 源码解读：http://www.lxway.com/14190441.htm
...
var
	...
	// List of deleted data cache ids, so we can reuse them 删除的数据缓存系统的列表，所以我们可以重用它们
	core_deletedIds = [],

	core_version = "2.0.3",

	// Save a reference to some core methods  保存核心方法的引用 
	...
	/**
	 * 调用 jquery是通过$()来访问的，也就是说$()返回的是jquery实例。
	 * 所有调用jquery是返回的jquery实例。
	 */
	...
	/* 
	 * 第一：\w匹配包含下划线的任何单词字符，等价于[A-Za-z0-9_]   []字符集合：匹配所包含的任意一个字符      *匹配前面的子表达式0次或多次
	 *     #([\w-]*) 表示以 #号开头  包含任意字符和任意个  - 组成的字符串   用来匹配   #id
	 * 第二：\s 表示匹配任何空白字符，包含空格、制表符、换页符等。[\f\n\r\t\v] 表示匹配的字符串之前可以有空格
	 * 
	 *     * 表示匹配前面的子表达式零次或多次
	 *    \W 表示匹配任何非单词字符，等价于[^A-Za-z0-9_]
	 *     + 表示匹配前面的子表达式一次或多次
	 * 
	 *    所以：(<[\w\W]+>) 表示字符串以包含在<>中 并且<>中有单词字符或非单词字符一次或多次，可以匹配html标签 包含自定义标签
	 * 
	 * 	  [^>]表示匹配不包含>的任意(一个)字符	   
	 * 
	 * 	  x|y  |  表示匹配x或者y	
	 * 第三： ?:pattern 匹配pattern但不获取匹配结果，这是一个非获取匹配。不保存子正则表达式匹配的内容
	 * 
	 * 个人理解：该正则表达式的意思就是：匹配selector中 | 前半部分或者后半部分。并且如果匹配到前半部分，将匹配结果数组中1位置有值。如果匹配
	 * 到后半部分，则匹配结果数组中2位置有值
	 */
	rquickExpr = /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]*))$/,

	//匹配独立的标签 \1表示反向引用  表示第一个括号匹配的内容 该正则表达式可匹配   <img />  或者  <div></div>
	rsingleTag = /^<(\w+)\s*\/?>(?:<\/\1>|)$/,
...
jQuery.fn = jQuery.prototype = {
	...
	constructor: jQuery,
	//此方法在引入jquery时，调用7次 其中一次rootjQuery输出为undefined  特此记录
	init: function( selector, context, rootjQuery ) {
		var match, elem;
		//如果调用jquery方式为: $(""), $(null), $(undefined), $(false)此四种为，返回[]
		if ( !selector ) {
			return this;
		}
		...
		if ( typeof selector === "string" ) {
			//如果用户selector是html标签的话，则直接返回[ null, selector, null ]不用进行正则匹配
			if ( selector.charAt(0) === "<" && selector.charAt( selector.length - 1 ) === ">" && selector.length >= 3 ) {
				match = [ null, selector, null ];
			} else {
				//此处获取的match是一个数组,如：["#pid", undefined, "pid", index: 0, input: "#pid"]
				/* /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]*))$/
				 * match为返回的匹配数组，如果selector匹配到\s*(<[\w\W]+>)[^>]*（也就是匹配到标签） 则在match[1]中有值，
				 * 如果selector匹配到#([\w-]*) (也就是匹配到#id)  则在match[2]中有值 
				 */
				match = rquickExpr.exec( selector ); 
			}
			//Match html or make sure no context is specified for #id
			/* 
			 * 在没有指定上下文环境的情况下，返回符合正则表达式 #id  或     html5标签   的页面元素对象
			 */
			if ( match && (match[1] || !context) ) {
				...
				if ( match[1] ) {
					...
					// scripts 为 back-compat（兼容模式关闭）时，script为true 
					//合并数组，根据selector匹配的标签，创建新的标签
...
/*
 * 通过原型传递，将jquery的原型传递给jQuery.prototype.init.prototype
 * fn为指向jquery原型的引用
 */
...
jQuery.extend({
	...
	//用于判断指定参数是否是一个纯粹的对象，通过{}或new Object的
	...
	isEmptyObject: function( obj ) {
		//如何obj为undefined 则返回true，否则返回false
		var name;
		for ( name in obj ) {
			return false;
		}
	...
	// data: html标签字符串，如<div>
	// context (optional): 如果指定，将在本上下文中创建该片段，默认为document
	// keepScripts (optional): keepScripts为 true, 在html字符串中将包含script脚本
	parseHTML: function( data, context, keepScripts ) {
		...
		//匹配单一标签，如：<div></div>会通过正则匹配
		var parsed = rsingleTag.exec( data ),
			scripts = !keepScripts && [];

		//单标签  如果匹配单一标签成功，则创建该节点并返回
		if ( parsed ) {
			return [ context.createElement( parsed[1] ) ];
		}

		//当输入的字符串不为不光是标签时，会创建片段，没有看懂。???
...
jQuery.extend({
	...
	Deferred: function( func ) {
	  /**
        * 创建一个数据元组集,每个元组分别包含一些与当前委托人(deferred)相关的信息: 
        * 
        ...
        */
		var tuples = [
				...
				// action 执行状态, add listener 添加侦听器（事件处理函数）,
				// listener list 侦听器列表(事件处理函数列表),
				// final state 最终状态
			...
			/* 
			 * 委托人三种状态，（deferred的状态）分为三种：
			 * pending(挂起状态) resolved(完成状态) rejected（失败状态）
			 */
			state = "pending",
			/**
...
jQuery.fn.extend({
	queue: function( type, data ) {//$(document).queue("q1",aaa);
		...
		return data === undefined ?
			...
			this.each(function() {//这里就是设置操作，对每个元素都进行设置
				//入队操作，会在缓存系统中添加一个队列q1，队列中，入队aaa。
	 ...
	 *我们先举个例子说下这个方法的作用：$(this).animate({width:300},2).animate({left:300},2);
	...
	buildFragment: function( elems, context, scripts, selection ) {
		...
		for ( ; i < l; i++ ) {
			...
			if ( elem || elem === 0 ) {
				...
				/* 
				 *   rhtml = /<|&#?\w+;/ 表示匹配  <  或则以&开头;结尾          中间可以有0个或多个# 以及一个或多个 包括下划线的字符
				 *   将非html字符串转换为文本结点
				 */
				...
				//转换成DOM结点
					...
					console.log(rtagName.exec( elem ));
...
//Animation模块
var fxNow,//动画开始时间 
	timerId,// timerId使用一个ID来执行动画setInterval
...
function Animation( elem, properties, options ) {
	var result,
		...
		//首先,初始化一个延时对象，这个延时对象用来删除elem元素
		deferred = jQuery.Deferred().always( function() {
			// don't match elem in the :animated selector
			delete tick.elem;
		}),
		/* 
		 * 然后，生成一个每一个时间点（相邻两个时间点的事件间隔默认为13毫秒）上都会执行的函数tick，
		...
		tick = function() {
			...
			var currentTime = fxNow || createFxNow(),// 计算当前动画时间戳
				//结束时间减当前时间，计算出剩余时间
			...
			//如果执行进度未完成且tweens数组有元素,返回剩余时间
			if ( percent < 1 && length ) {
				return remaining;
			} else {
				//动画执行完毕，执行所有延时队列中的函数（包括清除动画相关的数据）
				//否则表示已完成，触发resolve回调列表，并返回false值
...
function defaultPrefilter( elem, props, opts ) {
	...
	for ( prop in props ) {
		...
		if ( rfxtypes.exec( value ) ) {
			...
			// 将需要show/hide/toggle的属性保存到orig数组中
	...
	if ( !jQuery.isEmptyObject( orig ) ) {
		...
		for ( prop in orig ) {
			// 创建Tween实例     dataShow[ prop ]获取元素原始样式值
...
/* 
 * Tween.propHooks.scrollTop 和Tween.propHooks.scrollLeft
 * 两个主要是在ie8离线状态下会出现混乱而把css特征值保存到节点上
 */
...
//每个时间点都会执行的函数外壳，会取出jQuery.timers中的函数执行  // setInterval回调
